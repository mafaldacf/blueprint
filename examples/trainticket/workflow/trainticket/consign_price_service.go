...
func NewConsignPriceServiceImpl(ctx context.Context, db backend.NoSQLDatabase) (ConsignPriceService, error) {
...
func (c *ConsignPriceServiceImpl) GetPriceInfo(ctx context.Context) (string, error) {
	coll, err := c.db.GetCollection(ctx, "consignprice_db", "consign")
	if err != nil {
		return "", err
	}
	query := bson.D{{Key: "Index", Value: 0}}
...
func (c *ConsignPriceServiceImpl) CreateAndModifyPriceConfig(ctx context.Context, priceConfig ConsignPrice) (ConsignPrice, error) {
	...
	priceConfig.Index = 0
	if exists {
		ok, err := coll.Upsert(ctx, bson.D{{Key: "Index", Value: 0}}, priceConfig)
		if err != nil {
