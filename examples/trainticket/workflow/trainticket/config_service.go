...
func NewConfigServiceImpl(ctx context.Context, db backend.NoSQLDatabase) (ConfigService, error) {
	return &ConfigServiceImpl{db: db}, nil
}

func (c *ConfigServiceImpl) Create(ctx context.Context, conf Config) error {
	coll, err := c.db.GetCollection(ctx, "config_db", "config")
	if err != nil {
		return err
	}
	query := bson.D{{Key: "Name", Value: conf.Name}}
