...
func NewStationFoodServiceImpl(ctx context.Context, db backend.NoSQLDatabase) (StationFoodService, error) {
...
func (s *StationFoodServiceImpl) CreateFoodStore(ctx context.Context, store StationFoodStore) error {
	coll, err := s.db.GetCollection(ctx, "stationfood_db", "stationfood")
	if err != nil {
		return err
	}
	query := bson.D{{Key: "ID", Value: store.ID}}
...
func (s *StationFoodServiceImpl) ListFoodStoresByStationName(ctx context.Context, station string) ([]StationFoodStore, error) {
	...
	query := bson.D{{Key: "StationName", Value: station}}
