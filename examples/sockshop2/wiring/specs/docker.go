...
import (
	...
	"github.com/blueprint-uservices/blueprint/examples/sockshop2/workflow/frontend"
	...
	"github.com/blueprint-uservices/blueprint/examples/sockshop2/workflow/queuemaster"
...
func makeDockerRabbitSpec(spec wiring.WiringSpec) ([]string, error) {
	...
	shipdb := mongodb.Container(spec, "shipdb")
	shipqueue := rabbitmq.Container(spec, "shipqueue", "shipqueue")
	...
	frontend_service := workflow.Service[frontend.FrontendService](spec, "frontend", user_service, catalogue_service, cart_service, order_service)
