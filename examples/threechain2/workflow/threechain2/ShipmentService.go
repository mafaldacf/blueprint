...
func (s *ShipmentServiceImpl) createPendingShipment(ctx context.Context, shipmentMessage ShipmentMessage) error {
	collection, _ := s.shipment_db.GetCollection(ctx, "shipment_database", "shipment_collection")
	shipment := Shipment{
		OrderID:  shipmentMessage.OrderID,
		Username: shipmentMessage.Username,
		Status:   "pending",
	}
