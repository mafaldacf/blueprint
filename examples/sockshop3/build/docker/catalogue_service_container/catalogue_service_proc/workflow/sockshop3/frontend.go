...
func NewFrontend(ctx context.Context, user UserService, catalogue CatalogueService, cart CartService, order OrderService) (FrontendService, error) {
	f := &FrontendImpl{
		user:      user,
		catalogue: catalogue,
		cart:      cart,
...
func (f *FrontendImpl) UpdateItem(ctx context.Context, sessionID string, itemID string, quantity int) (string, error) {
	...
	return sessionID, f.cart.UpdateItem(ctx, sessionID, Item{ID: item.ID, Quantity: quantity, UnitPrice: item.Price})
}
