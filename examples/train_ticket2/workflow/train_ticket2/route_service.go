...
func (r *RouteServiceImpl) DeleteRoute(ctx context.Context, id string) error {
	coll, err := r.db.GetCollection(ctx, "route_db", "route")
	if err != nil {
		return err
	}
	return coll.DeleteOne(ctx, bson.D{{Key: "ID", Value: id}})
}
...
func (r *RouteServiceImpl) GetRouteById(ctx context.Context, id string) (Route, error) {
	...
	res, err := coll.FindOne(ctx, bson.D{{Key: "ID", Value: id}})
	if err != nil {
		return Route{}, err
	}
	var route Route
...
func (r *RouteServiceImpl) GetRouteByStartAndEnd(ctx context.Context, start string, end string) (Route, error) {
	...
	query := bson.D{{Key: "$and", Value: bson.A{
		bson.D{{Key: "StartStation", Value: start}},
		bson.D{{Key: "EndStation", Value: end}},
	}}}
...
func (r *RouteServiceImpl) CreateAndModify(ctx context.Context, info RouteInfo) (Route, error) {
	...
	if info.ID == "" || len(info.ID) < MAXIDLEN {
		...
		res, err := coll.FindOne(ctx, bson.D{{Key: "ID", Value: info.ID}})
		if err != nil {
			return route, err
		}
		var old_route Route
	...
	if old_exists {
		ok, err := coll.Upsert(ctx, bson.D{{Key: "ID", Value: info.ID}}, route)
		if err != nil {
			return route, err
		}
		if !ok {
