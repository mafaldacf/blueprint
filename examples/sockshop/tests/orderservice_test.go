...
import (
	...
	order "github.com/blueprint-uservices/blueprint/examples/sockshop/workflow/orders"
...
func init() {
	...
	ordersRegistry.Register("local", func(ctx context.Context) (order.OrderService, error) {
		...
		return order.NewOrderServiceImpl(ctx, user, cart, payment, shipping, orderdb)
	})
}
