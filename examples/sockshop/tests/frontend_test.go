...
import (
	...
	cart "github.com/blueprint-uservices/blueprint/examples/sockshop/workflow/carts"
...
func init() {
	...
	frontendRegistry.Register("local", func(ctx context.Context) (frontend.Frontend, error) {
		...
		return frontend.NewFrontendImpl(ctx, user, catalogue, cart, order)
	})
}
...
func TestFrontend(t *testing.T) {
...

	{
		...
		for _, sock := range items {
			sock2, err := fe.GetSock(ctx, sock.SockID)
			require.NoError(t, err)
...

	{
		...
		sessionID, err := fe.AddItem(ctx, "", items[0].SockID)
		require.NoError(t, err)
		...
		// Get the cart; should have 1 item
		{
			...
			require.NoError(t, err)
			require.Len(t, crt, 1)
			require.Contains(t, crt, cart.Item{ID: items[0].SockID, Quantity: 1, UnitPrice: items[0].Price})
		}

		// Add a few more socks
		for i := 0; i < 3; i++ {
			newSessionID, err := fe.AddItem(ctx, sessionID, items[0].SockID)
			require.NoError(t, err)
			require.Equal(t, sessionID, newSessionID)
		}

		{
			newSessionID, err := fe.AddItem(ctx, sessionID, items[3].SockID)
			require.NoError(t, err)
			require.Equal(t, sessionID, newSessionID)
		}

		{
			...
			require.NoError(t, err)
			require.Len(t, crt, 2)
			require.Contains(t, crt, cart.Item{ID: items[0].SockID, Quantity: 4, UnitPrice: items[0].Price})
			require.Contains(t, crt, cart.Item{ID: items[3].SockID, Quantity: 1, UnitPrice: items[3].Price})
		}
		...
		{
			newSessionID, err := fe.UpdateItem(ctx, userSessionID, items[0].SockID, 2)
			require.NoError(t, err)
...

		{
			...
			require.NoError(t, err)
			require.Len(t, crt, 2)
			require.Contains(t, crt, cart.Item{ID: items[0].SockID, Quantity: 2, UnitPrice: items[0].Price})
			require.Contains(t, crt, cart.Item{ID: items[3].SockID, Quantity: 1, UnitPrice: items[3].Price})
		}
